...
def admin_dashboard(db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    pending_jobs  = db.query(func.count(GenerationJob.id)).filter(
        GenerationJob.status == "pending").scalar()
    failed_jobs   = db.query(func.count(GenerationJob.id)).filter(
        GenerationJob.status == "failed").scalar()
    total_revenue = db.query(func.sum(Transaction.amount)).filter(
        Transaction.amount > 0).scalar() or 0.0
    recent_jobs   = db.query(GenerationJob).order_by(
        GenerationJob.created_at.desc()).limit(20).all()
    return {
        "stats": {
        ...
        "recent_jobs": [
            {"id": j.id, "type": j.job_type, "status": j.status,
             "user_id": j.user_id, "cost": j.cost, "created_at": j.created_at}
...
def list_users(skip: int = 0, limit: int = 50,
    ...
    return {"users": [
        {"id": u.id, "email": u.email, "is_blocked": u.is_blocked,
